...
for file in $(find macpf/keywords/ -iname *csv*); do echo $file; cp $file .; done
...
# Running script
python3 ~/Development/csv_utils/csv_cleaner.py -i macpf/1_combined/1.1_macpf_combined.csv -o macpf/1_combined/1.2_macpf_cleaned.csv
```
...
find macpf/keywords/ -iname *protein* | xargs cat > macpf/1_combined/1.3_protein_combined.fasta
```
...
python3 ~/Development/HMMER_API/hmmer_api_v4.py -seq ../2_name_copies/2.1_3_macpf_protein_nonamedupes.fasta
...
```bash
python3 ~/Development/HMMER_api_json_to_tab/json_to_tab.py -dir hmmer_results/
```
...
python3 ~/Development/NCBI_taxonomy/taxonomy_v7_pandas_gemini.py -i ../data/basidiomycota_for_taxonomy.txt -o basidiomycota_phylogeny/basidiomycota_taxonomy.csv
...
python3 ~/Development/stat/stat_v2.py -m macpf/3_hmmer/3_4_after_hmmer/3.4.5_macpf_after_hmmer.csv -p basidiomycota_phylogeny/basidiomycota_taxonomy.csv -o macpf/3_hmmer/3_4_after_hmmer/3.6.3_macpf_statistics_all.xlsx
```
...
grep -v -F -f macpf/not_allowed.txt macpf/3_hmmer/3_4_after_hmmer/3.4.5_macpf_after_hmmer.csv >> macpf/4_permitted/4.1_macpf_permitted_metadata.csv
```
...
seqkit rmdup -s -D macpf/5_final/5.1_duplicate_sequences_seq.txt -d macpf/5_final/5.1_duplicate_sequences_seq.fasta  macpf/4_permitted/4.2_macpf_permitted.fasta > macpf/5_final/5.1_macpf_final.fasta
```
...
```bash
# Get header
head -1 macpf/4_permitted/4.1_macpf_permitted_metadata.csv > macpf/5_final/5.2_macpf_final_metadata.csv

# Get fasta headers from fasta file
seqkit seq -n macpf/5_final/5.1_macpf_final.fasta > macpf/5_final/5.2_macpf_final_headers.txt

grep -f macpf/5_final/5.2_macpf_final_headers.txt macpf/4_permitted/4.1_macpf_permitted_metadata.csv >> macpf/5_final/5.2_macpf_final_metadata.csv
```
...
seqkit stats -a -T macpf/5_final/5.1_duplicate_sequences_seq.fasta > macpf/seqkit_stats/5.1.1_macpf_seqdups_stats.tsv
...
```
seqkit stats -a -T macpf/5_final/5.1_macpf_final.fasta > macpf/seqkit_stats/5.1.2_macpf_final_stats.tsv
```
...
```bash
# Checking and exporting number of sequences in csv file
num_seqs=$(tail -n +2 macpf/5_final/5.2_macpf_final_metadata.csv | cut -d "," -f 2 | wc -l); echo "num_sequences_csv: ${num_seqs}" > macpf/logs/5.1.3_log.txt


# Exporting number of sequences in seqkit stats file
num_seqs=$(cat macpf/seqkit_stats/5.1.2_macpf_final_stats.tsv | csvtk cut -t -f "num_seqs" | csvtk del-header); echo "num_seqs_fasta: ${num_seqs}" >> macpf/logs/5.1.3_log.txt
...
```bash
python3 ~/Development/stat/stat_v2.py -m macpf/5_final/5.2_macpf_final_metadata.csv -p basidiomycota_phylogeny/basidiomycota_taxonomy.csv -o macpf/5_final/5.3_macpf_final_statistics.xlsx		
```

### 5.4 združimo datoteko z metapodatki in filogenijo za lažje delo pri uvažanju podatkov v TreeViewer.
